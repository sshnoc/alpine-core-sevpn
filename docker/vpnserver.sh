...
function init_certificate() {
  ...
  local cmd_file=${DATA_DIR}/certificate.cmd
...
function init_wireguard() {
  ...
  local cmd_file=${DATA_DIR}/wireguard.cmd
  local is_cmd=false
...
function init_vpnserver() {
  while true ; do
    ...
    netstat -tan| grep ${VPN_SERVER_PORT} &> /dev/null
    if [ $? -gt 0 ] ; then
        sleep 2
    else
      break
  ...
  if [ -r ${password_cmd} ] ; then
    ...
    _info "Admin password set"
  fi

  local certificate_cmd=${DATA_DIR}/certificate.cmd
  if [ -r ${certificate_cmd} ] ; then
    vpn_server_cmd /IN:${certificate_cmd} &> /dev/null
    _info "Server certificate loaded"
  fi

  local wireguard_cmd=${DATA_DIR}/wireguard.cmd
  if [ -r ${wireguard_cmd} ] ; then
    vpn_server_cmd /IN:${wireguard_cmd} &> /dev/null
    _info "Wireguard credentials loaded"
  fi

  _info "VPN Server Init finished"
}
